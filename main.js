...
//   .then((result) => {
//     for (let data in result) jsonFileData.push(result[data]);
//   });
// let jsonFileData = [];
...
function sortByText(text) {
  if (text === "yes") {
    for (let i = 0; i < jsonFileData.length; i++) {
      const getText = jsonFileData[i].reviewFullText;
      if (getText === "") {
        let getIndex = jsonFileData.indexOf(jsonFileData[i]);
        const elements = jsonFileData.splice(getIndex, 1);
        jsonFileData.push(elements[0]);
      }
    }
  } else {
    return jsonFileData;
...
function sortByRating(ratingText) {
  if (ratingText === "high") {
    jsonFileData.sort((a, b) => parseFloat(b.rating) - parseFloat(a.rating));
  } else {
    jsonFileData.sort((a, b) => parseFloat(a.rating) - parseFloat(b.rating));
  }
}

function sortByDate(date) {
  if (date != "new") {
    jsonFileData.sort(function (a, b) {
      return new Date(a.reviewCreatedOnDate) - new Date(b.reviewCreatedOnDate);
    });
  } else {
    jsonFileData.sort(function (a, b) {
      return new Date(b.reviewCreatedOnDate) - new Date(a.reviewCreatedOnDate);
    });
  }
}

function sortByNumRating(num) {
  for (let i = 0; i < jsonFileData.length; i++) {
    if (num <= jsonFileData[i].rating) {
      let newRow = table.insertRow(-1);
      newRow.insertCell(0).append(jsonFileData[i].reviewFullText);
      newRow.insertCell(0).append(jsonFileData[i].reviewCreatedOnDate);
      newRow.insertCell(0).append(jsonFileData[i].rating);
...
document.getElementById("form").onclick = function (e) {
  ...
  let priorText = document.getElementById("texts").value;
  let ratingText = document.getElementById("ratings").value;
  let date = document.getElementById("dates").value;
  let numberRating = document.getElementById("minRatings").value;
  sortByText(priorText);
  sortByRating(ratingText);
  sortByNumRating(numberRating);
  sortByDate(date);
